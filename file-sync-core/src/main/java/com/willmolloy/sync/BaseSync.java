...
import com.willmolloy.sync.util.concurrent.VoidStructuredTaskScope;
...
public abstract class BaseSync<SourceFileT extends File, DestFileT extends File>
  ...
  private void executeDeletes(
    ...
    try (VoidStructuredTaskScope scope = new VoidStructuredTaskScope("delete")) {
      destFileTree
          .postorder()
  ...
  private void executePuts(
      ...
      FileTree<DestFileT> destFileTree,
      Statistics<SourceFileT, DestFileT> statistics) {
    try (VoidStructuredTaskScope scope = new VoidStructuredTaskScope("put")) {
      sourceFileTree
