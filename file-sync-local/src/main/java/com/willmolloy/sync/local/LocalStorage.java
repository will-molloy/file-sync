...
public final class LocalStorage implements Location<LocalFile> {
  ...
  public FileTree<LocalFile> scan() {
    try {
      ...
      try (ForkJoinPool pool = forkJoinPool()) {
        ParallelWalk walk = new ParallelWalk(rootDir, consumer);
        pool.invoke(walk);
      }
  ...
  private static final class ParallelWalk extends RecursiveAction {
    private final Path dir;
    private final BiConsumer<Path, BasicFileAttributes> consumer;

    private ParallelWalk(Path dir, BiConsumer<Path, BasicFileAttributes> consumer) {
      this.dir = dir;
    ...
    protected void compute() {
      List<ParallelWalk> forks = new ArrayList<>();
      try {
        Files.walkFileTree(
            dir,
            new FileVisitor<>() {
              ...
              public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attributes) {
                if (ParallelWalk.this.dir == dir) {
                  ...
                  ParallelWalk fork = new ParallelWalk(dir, consumer);
                  fork.fork();
                  forks.add(fork);
